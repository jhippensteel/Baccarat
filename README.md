# Baccarat Desktop Application

## Primary Author: Jaysen R Hippensteel

## Last Modified: 1/12/26

This repository contains source code for a Java based Baccarat desktop application. Utilizes JavaFX library for GUI implementation and Apache Maven for dependency management.

Copyright (C) 2026 Jaysen Hippensteel

*You may use, distribute and modify code within this repository under the terms of GNU General Public License, Version 3, 29 June 2007.*
*Please visit [the GNU website](https://gnu.org/licenses/gpl-3.0.en.html)*